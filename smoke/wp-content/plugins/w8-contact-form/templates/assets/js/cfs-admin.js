jQuery(document).ready(function() {
	...
	var selected_contact = "", rmdni = false, protocol, checker;
	cfsadmin( function() {
	...
	initialize_tooltips();
	cfsadmin("#add-contact").on( "click", function(){
	var counter = parseInt(cfsadmin(".one-contact").length)+1;
		...
		cfsadmin(document).on( "click", "#contact" + counter + "-remove", function() { 
			cfsadmin( "#uploaded_contact" + counter ).html( "<div class=\"imageelement\"><div id=\"uploaded_contact" + counter + "\"><input id=\"contact" + counter + "-upload\" class=\"button add-button\" type=\"button\" value=\"" + cfs_admin_datas.languages.addimage + "\" /></div></div>");
			return false;
			});
		})
	})
	...
	cfsadmin( "#log-display" ).on( "click", function() {
				var data = {
			action: 'ajax_cfs',
			cfscmd: 'getlogs'
		};
	...
	cfsadmin("#save-contact").on( "click", function(){
		save_contacts();
	})
	 ...
	 if (document.getElementById) {
	  if (elem=document.getElementById(id)) {
	   if (elem.style) elem.style.backgroundColor=sColor;
		...
		cfsadmin('#picker'+num).on( "click", function(e) { // click event handler
			bCanPreview[num] = !bCanPreview[num];
	   });
		cfsadmin('.preview'+num).on( "click", function(e) { // preview click
